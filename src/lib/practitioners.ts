...
export type Profile = {
  id: string;
  /**
   * The public identifier — eight characters of Crockford base32, and the whole
   * of `/p/<handle>`.
   *
   * Separate from `id` rather than derived from it, which is the change #119
   * made. It used to be `id.slice(0, 6)`, computed here and enforced by nothing:
   * two profiles sharing those six characters served one of them under both
   * URLs, with the wrong person's credentials and the wrong person's badge. It
   * is a `not null unique` column now, generated by a Postgres default, so the
   * uniqueness is Postgres's rather than a comment's.
   *
   * `anon` is granted `select (handle)` and nothing else on it: it is absent from
   * every `insert` and `update` grant `authenticated` holds, and pinned by
   * `practitioners_guard` besides. A practitioner cannot move their own URL, and
   * nobody can take somebody else's.
   */
  handle: string;
  name: string;
 ...
 * the trailing short id resolving and the name as decoration, and the route
 * redirected a stale slug to the canonical path. All of that went with the
 * scheme it decorated: `profilePath` no longer reads the name, `findByHandle`
 * no longer splits on a hyphen, and the route no longer redirects, because
 * there is exactly one path per profile for it to redirect to.
 *
 * What that costs, recorded so it is a decision rather than an oversight: a bare
 * URL tells a hiring manager nothing before they click, and gives search engines
 * no name signal. What it buys is that dropping the slug is the direction that
 * stays open — the handle is the key either way, so a readable prefix can be
 * added later as pure decoration with every published `/p/<handle>` still
 * resolving, while removing slugs later would break every one of them.
 *
 * There is nothing to compute here any more, and that is the point rather than a
 * sign the function should be inlined: it is the one generator, and its being a
 * function is what stops a second one appearing beside it. There was briefly a
 * second scheme, in the prototype, that hashed the *name* into a short id — it
 * disagreed with production the moment both existed, the directory linking one
 * way and the page resolving the other, and it was deleted rather than
 * reconciled. Nothing about #119 makes that cheaper to repeat.
 */
export function profilePath(person: Pick<Profile, "handle">) {
  return `/p/${person.handle}`;
}

/**
 * Whether a string could be a handle at all: eight Crockford base32 characters,
 * lowercase, `i`/`l`/`o`/`u` excluded.
 *
 * **Not the enforcement.** `practitioners_handle_format` is, on the column, and
 * `public.new_profile_handle()` is what produces one. This is a cheap predicate
 * over untrusted text from a URL, so that `/p/../../etc` costs no round trip
 * before it 404s. Being a second copy of the format is the price of that, and
 * the direction the two can disagree in is safe: a handle this rejects and the
 * database would have accepted is a profile that 404s, never a wrong profile
 * served — which is the failure #119 removed.
 *
 * It lives beside `profilePath` rather than in `@/lib/directory` because the
 * generator and the validator of one format belong together, and because a pure
 * predicate can be asserted in `pnpm test` while a query cannot.
 */
export function isProfileHandle(handle: string) {
  return /^[0123456789abcdefghjkmnpqrstvwxyz]{8}$/.test(handle);
}
