...
What does not exist yet: `practitioner_credentials`, `catalogue_guard`, `correct_catalogue_entry()` and the three `clear_profile_verification()` triggers (all #50), `practitioner_services` (#90), auth in the application, and the hosted project's copy of the hook setting. `catalogue_guard` is also why `credential_catalogue.updated_at` is not maintained on update yet: the guard is what bumps it, its body reads `practitioner_credentials`, and a plpgsql body resolves its names at call time — so shipping it before that table exists would raise `relation does not exist` on every update to the catalogue. The token-side machinery landing is not the app having auth — there is no `@supabase/ssr` client and no sign-in flow, and every policy in the spec is written against `auth.uid()`, so none of them is reachable from the app until #14. The rest of this section is the contract for building those — treat it as binding, not as a description of current state.
...
A push to `main` does not deploy directly — `vercel-deploy.yml` triggers on `workflow_run` from **both** `CI` and `Schema`, so nothing ships until `Quality` and the schema suite are green at that commit. Note what naming two workflows in a `workflow_run` trigger actually does: it fires once per workflow, not once when both have finished. The ordering comes from a gate step that reads the sibling workflow's run at the same commit and stands down unless it has finished — so whichever event lands last is the one that deploys. Two things follow. A documentation-only merge deploys off `CI` alone, because `Schema`'s `paths-ignore` means no run exists to wait for, and an absent run is treated as nothing to wait for rather than as a failure — which is also what keeps deploys working while `Schema` is toggled off by hand. And a red `Schema` stops the deploy silently: the job stands down rather than failing, because the `Schema` run is already reporting the failure and a second red X adds nothing.
