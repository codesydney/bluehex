...
- **`tests/db/`** — the schema's invariants, asserted against the local Supabase stack through PostgREST and through Postgres directly. `pnpm test:db` runs this project alone. **CI runs it in `.github/workflows/schema.yml`, not in the required `Quality` check** — a separate workflow because it boots five containers before it asserts anything, and the check that gates every merge should stay fast. Locally, `pnpm db:start` first, or the fixtures cannot be built and the run fails — though note the shape of it: the file that needs no stack still passes and the rest are reported *skipped* rather than failed, so read the exit code rather than the summary line. That shape is why the CI step is a bare `pnpm test:db` with nothing piped onto it: a pipeline would report the last command's status and a run that booted nothing would go green.
- **`e2e/`** — **Playwright**, which is not a Vitest project at all.

Putting a Vitest file anywhere else means it is silently never run.

**Database tests are not application code**, which is why `tests/db` is a project of its own rather than a directory under `src/`: they need a running stack, longer timeouts than a unit test has any use for, and serial execution, because they mutate global state — grants, roles, rows in `auth.users` — that no two files may hold at once. Keeping them out of `pnpm test` is also what stops a stopped Docker daemon from failing the suite that gates every merge — `Quality` needs no Docker, and `Schema` brings its own.
...
- Supabase (Postgres) through `@supabase/supabase-js`, and `@supabase/ssr` for the cookie-backed clients that carry a session; local stack via the Supabase CLI
...
- `src/app/profile/_lib/` and `src/app/admin/_lib/` — the two halves of self-service, each beside the route that uses it and neither reachable from a client component. `profile-read.ts` is the practitioner's own read (two `security definer` RPCs and three ordinary selects), `actions.ts` is the Server Action that writes it back, and `profile-mapping.ts` and `profile-plan.ts` are the pure halves — rows to draft, and draft to the inserts, updates and deletes a save makes on the child tables. The admin side is the same shape against `bluehex_admin`.
- `src/app/contact/_lib/enquiry.ts` — the pure half of the enquiry form: the EmailJS ids, the mapping of a filled-in form onto the four variables the shared Code.Sydney template renders, the `mailto:` the failure path falls back to, and the one `fetch` that sends. It is browser code, unlike the other two `_lib` directories — the send happens from the visitor's browser and there is no server-side send and no secret. See `docs/adr/0003-enquiries-send-through-emailjs.md`.
...
- `src/lib/` — data and configuration, no rendering. `site.ts` is the single source of truth for naming, nav, contact details and legal links. `profile-draft.ts` is the editor's model — the practitioner-writable field set, the derivations over it and `toWritePayload`, which is where `"" → null` and `"" → do not submit this row` happen; `profile-validation.ts` is what the form checks before any of it travels, and `profile-save.ts` is the two types the page and the client editor share so a `"use server"` module does not have to export one. `practitioners.ts` is the **public view model** — the shapes `anon` is granted, the two derivations and the two string helpers, and no rows since #53. `directory.ts` is the read that fills it and `directory-mapping.ts` is the pure half of that read, split so the ordering and null rules can be asserted without a stack. `database.types.ts` is **generated** — regenerate it with `pnpm db:types` after a migration rather than editing it.
- `src/lib/supabase/` — **three** clients, and which one you reach for is a correctness question rather than a style one. `anon.ts` is the module-scoped anonymous client, cached for the life of the process and therefore never allowed to hold a session; `server.ts` builds one client per server request from that request's cookies; `browser.ts` is the singleton the browser signs in through. `env.ts` reads the two environment variables, at call time so `next build` survives their absence.
- `src/lib/auth/` — who is asking. `claims.ts` reads the `bluehex_admin` role off a verified token, `routes.ts` is the pure decision about which paths need what, `session.ts` is the server-side guard a protected page opens with (`requireAccount` / `requireAdmin`), and `actions.ts` holds the sign-out Server Action. None of it authorises anything — Postgres does that.
- `src/proxy.ts` — refreshes the Supabase session on every matched request and turns signed-out visitors away. **`proxy.ts`, not `middleware.ts`**: Next.js 16 renamed the convention and only one such file is allowed per project.
...
**Real people only** — no placeholder profiles. `practitioners.ts` used to enforce that by shipping an empty array; since #53 the directory reads Postgres, so the rule moved to where the rows are: a profile reaches the hosted project when the person has agreed to be published. The directory still renders an invitation card for the empty slots rather than inventing entries, and `supabase/seed.sql` is the one place invented people are allowed, because it never runs against hosted.
...
Neither is stored. `verified` is a column on `practitioner_credentials` and per credential; the profile-level badge is `hasVerifiedBadge` rolling those rows up, and `certified` is `isCertified` asking whether any of them names a catalogue entry with `kind = 'certification'`. Both live in `src/lib/practitioners.ts`, both are derived on every render, and neither has a column to disagree with. The separation of the two *ideas* is the invariant; a column for either is not. See `docs/spec/profile-and-credentials.md`.
...
**Magic link only.** One address, one emailed link, and that link both creates the account and signs it in — `signInWithOtp` with `shouldCreateUser: true`. What follows from that is worth stating because its absence looks like an omission: **there is no password, so there is no password reset, and there is no separate sign-up form.** A second form would only be able to tell the visitor whether an address is already registered, which is a question nobody should be able to ask. No OAuth provider is offered; GitHub is the plausible one for this audience and is a later decision, not a missing step.

**Three clients, and the module-scoped one must never hold a session.** `src/lib/supabase/anon.ts` is cached for the life of the process, so a session stored on it would be every visitor's session. Signed-in reads get a fresh client per request from `server.ts`, and the browser gets the singleton in `browser.ts`. This is the failure that would not announce itself, which is why `anon.ts` sets `persistSession: false` rather than relying on nobody reaching for the client that already exists.

**`src/proxy.ts` is not optional plumbing.** An access token lasts an hour, and a Server Component can read cookies but cannot write them — HTTP will not take a `Set-Cookie` once a response has begun streaming. So the refresh has to happen before the render, which is what the proxy is for. Dropping a route from its matcher does not fail; it means sessions on that route stop refreshing, and the symptom is somebody signed out an hour later for no visible reason.

**Protecting a route takes one line.** A page under a gated root opens with `await requireAccount("/its/own/path")`, or `requireAdmin` for an admin one, from `src/lib/auth/session.ts`; both return the `Viewer` or redirect. The roots themselves are `ACCOUNT_ROOTS` and `ADMIN_ROOTS` in `src/lib/auth/routes.ts`, and adding a route beneath an existing root needs no change there. The guard and the proxy share one decision function so they cannot drift.

**The guard is not the control.** Every admin privilege belongs to the `bluehex_admin` Postgres role and is checked by Postgres on every statement, so deleting `requireAdmin` from a page would make it ugly rather than insecure. Reading the claim is presentation — see `docs/adr/0001-admins-are-a-postgres-role.md`, and note that it is a snapshot of a token: removing someone from `admins` takes effect on their next refresh, not immediately. The lever for cutting an admin off now is revoking their sessions.

**A `next` parameter is attacker-controlled.** It arrives in a query string and is carried into an emailed link, so it goes through `safeReturnTo` before anything follows it — otherwise a link could hand a visitor to another origin from ours, immediately after they authenticated. `//evil.example` and `/\evil.example` both read as paths and are not.
...
- **A new table is invisible to the API until you `grant` on it, and the failure looks
  ...
  **The second set is a setting, not a property of Postgres.** `api.auto_expose_new_tables` in `supabase/config.toml` is what makes the CLI revoke those default privileges when the stack starts, and it is pinned to `false` there because the meaning of leaving it *unset* flipped between CLI 2.114 (unset means `false`) and 2.116 (unset means `true`) — a Dependabot bump of the CLI started every `postgres`-owned table readable and writable by `anon` on a fresh stack, and the only table that noticed was `public.admins`, because it was the only one whose migration relied on the default instead of writing `revoke all … from anon, authenticated` before its grants. It does now, and `tests/db/default-privileges.test.ts` creates a table and asserts the API roles hold nothing on it, so a bump that stops honouring the pin fails the `Schema` workflow rather than the badge. The CLI says the field is removed on 2026-10-30; when that lands, that test is what says which way it went. The hosted project has the same switch as a dashboard toggle, "Default privileges for new entities", which `db push` does not carry — a table there starts with whatever that toggle says, which is the other reason every migration revokes first.
...
- **`docs/adr/0003-enquiries-send-through-emailjs.md`** — why the contact form sends from the browser through Code.Sydney's EmailJS account rather than through a route handler and a mail provider. Not a schema decision, so it is out of place in this list, but it is where the ADRs are indexed. Read it before adding a mail credential to this repository — avoiding the first secret is the whole argument.
...
Target is Vercel, deployed from the `main` branch of `codesydney/bluehex`. Pushes to `main` ship to production, and **production is the only deployed environment there is.** Vercel's Git integration is not in use, so a pull request gets CI and nothing else — no preview deployment, no preview URL, and no preview environment holding a second copy of the environment variables. What follows is that anything depending on the hosted project is proved for the first time in production: there is no rehearsal to catch a query that passes against the local stack and fails against the hosted one. The preview-database question that falls away with it is taken up under **How schema reaches production** below.
...
**An absent run always ships, and that is the gate's soft spot.** It happens two ways and they are not equivalent. `Schema` filters documentation-only pushes out with `paths-ignore`, so a documentation merge deploys off `CI` alone — benign, and refusing would mean documentation could never reach production. But a workflow **disabled by hand** also produces no run, and then every code merge ships with no schema signal at all. Both still deploy, because standing down would freeze production for as long as the workflow stayed off; the second emits a `::warning::` naming the state, so the deploy log says it shipped unchecked rather than reporting "nothing to wait for". If you disable `Schema`, you have turned this gate off — the annotation is the only thing that will tell you.
...
**Two repository secrets and one repository variable, all account actions.** `SUPABASE_ACCESS_TOKEN` and `SUPABASE_DB_PASSWORD` are secrets, read by the CLI itself so the password never reaches argv. `SUPABASE_PROJECT_REF` is a **variable**, because a project ref is not a secret: `NEXT_PUBLIC_SUPABASE_URL` is `https://<ref>.supabase.co` and is inlined into every production bundle by design, so registering it as a secret would buy nothing and would mask it as `***` in the log of the step that pushes. It is deliberately not called `SUPABASE_PROJECT_ID`, which the CLI reads as an override for `config.toml`'s `project_id` — the local Docker project, `bluehex`, not the hosted ref. The step checks all three and names whichever is missing, because an unset value arrives as an empty string and the CLI's error for that reads like a network problem. **Set all three before enabling the workflow**, because the check fails the step and the step is not optional to the job: a missing value stops production shipping at all, not just the migration, including a documentation merge with nothing to push.
...
**And disabling `Schema` now lets unchecked migrations through, not just unchecked code.** The gate ships on an absent sibling run either way — that is argued above and has not changed — but what it ships now includes a `db push`, and a migration recorded in the hosted `schema_migrations` cannot be amended the way an unshipped build can simply be re-run. Nothing refuses the push, because both alternatives are worse: skipping it would ship code ahead of its schema, which is the hazard this step exists to remove, and failing the job would freeze the documentation merges that same path was written to let through. The `::warning::` naming the disabled workflow is the whole of the mitigation, so read it as louder than it looks.
...
**Production is the only deployed environment.** The Vercel Git integration is not in use: every deploy this repository makes is this workflow's own `vercel deploy --prebuilt --prod`, and a pull request gets no deployment at all. So the hosted project has exactly two clients — this workflow, and a developer's local machine — and while that stays true the preview-database question #48 raises does not arise. What #14 has to decide is correspondingly narrower than that ticket assumes: not how to keep preview writes out of production data, but whether self-service writes want a second project once they land. #48 records the question; it does not yet record an answer.

`next build` passes with no environment variables set, which is what the lazy read in `src/lib/supabase/env.ts` is for and is worth keeping true. It is easier to break than it looks: a page that reads the environment *before* it awaits `cookies()` is still a candidate for prerendering, so the build reaches the throw and fails on a clone with no `.env.local`. `createServerSupabaseClient` awaits `cookies()` first for exactly that reason. It does not follow that a deployment can reach Supabase: `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` must be in Vercel's production environment — the only one, per above — or signing in fails. Both are set there as of August 2026. Note *where* it fails is not fixed: a request-time read fails at request time, but a statically rendered page runs its query on the build machine, so the same missing variable fails the build instead. Sign-in is the first feature that actually exercises this — no table is queried yet, but the Auth endpoint is, so a deploy that reaches production still proves nothing about the tables.
